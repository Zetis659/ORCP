...
![ ](https://drive.google.com/file/d/1jcdN8HX6GVnJ3ILpyZtcgjqevf91k8bX/view?usp=drive_link)
...
Для более подробного описания, а так же просмотра логики парсинга, откройте пункт [Data Collection](#data-collection)
...
В данном файле список всех брендов:
...
Если вы корректировали файлы с брендами, моделями и поколениями, при ошибках в парсинге, то нужно заменить эти файлы, на файлы из папки **BackUps**
...
После загрузки модели, помещаю их в Models/Classification/yolov8x-cls.pt - для классификации, и Models/Detection/yolov8x.pt - для детекции. Две модели нужны для того, чтобы сначала понимать, есть ли на изображении авто, а уже после производить его классификацию.
...
1. Постоянная обработка сообщений от пользователя, как текстовых, так нажатий на клавиатуры, а так же обработка фото
